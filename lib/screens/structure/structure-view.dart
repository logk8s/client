...
Widget boxedText(String text, Color color) {
  return Align(
    ...
    child: Text(text, textAlign: TextAlign.start, style: TextStyle(color: color )),
  );
}

Widget rectangleWidget(String text, Color bkgColor, Color textColor) {
  return InkWell(
    onTap: () {
      debugPrint('clicked');
    },
    child: Container(
        ...
        child: boxedText(text, textColor)
      ),
  );
}

Widget circleWidget(String text, Color bkgColor, Color textColor) {
  return InkWell(
    onTap: () {
      debugPrint('clicked');
    },
                        ...
                        builder: (Node node) {
                          ...
                          if (nodeKey.nodeType == NodeType.namespace) {
                            return rectangleWidget(
                                nodeKey.name, Colors.teal.shade200, Colors.teal.shade900);
                          }
                          if (nodeKey.nodeType == NodeType.pod) {
                            return circleWidget(
                                nodeKey.name, Colors.brown.shade200, Colors.brown.shade900);
                          }
                          if (nodeKey.nodeType == NodeType.container) {
                            return rectangleWidget(
                                nodeKey.name, Colors.brown.shade100, Colors.brown.shade800);
                          }
                          return circleWidget(
                              nodeKey.name, Colors.blueGrey.shade400, Colors.blue.shade50);
                        },
                      )),
                ),
              ),
